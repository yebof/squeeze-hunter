...
- **Round-13 corrections (Sep 2026).** These change numbers, not intent: (a) the trading calendar is the real NYSE schedule (`pandas_market_calendars`) everywhere — the federal calendar iterated Good Friday and skipped Columbus / Veterans Day; (b) Sortino uses the textbook downside deviation (RMS of `min(r, 0)` over all periods); (c) average payoff is a ratio of percentage returns per matched lot, not dollar P&L; (d) the captured-events forward allowance is one NYSE session after the event (the scan that sees the event fills at the next open); (e) the deflated-Sharpe benchmark is 0 for a single trial; (f) the backtest evaluates today's low against `max(running close-peak, today's open)` for the trailing stop, and a session with no bar carries the last mark forward instead of re-valuing at cost; (g) FINRA short-share counts are split-adjusted to today's share basis before dividing by Yahoo's float; (h) live exits are snapped to the minimum price variation, the pending-exit reconcile runs before the quote guard, a partially filled halve cancels its remainder, and a cancel must be terminal before a replacement is submitted; (i) IBKR account updates use the async subscription (the blocking call could not run inside the loop), `IBKR_ACCOUNT` must be one of the login's managed accounts, live mode requires an explicit non-paper `IBKR_PORT`, and a quote whose Ticker has not updated for 120 s is reported as no quote.
...
- **P2 Persistent state + reconciliation** *(done 2026-09-20)*. `store/state.py` writes an atomic JSON snapshot (`data.state_path`) after every job; startup restores it and reconciles against `IBroker.get_positions()`; the tick reconciles quantities every 60 s and EOD runs a full reconcile with an alert on drift; IBKR orders carry a client `orderRef` that is deduped on submit. Postgres can replace the JSON store behind the same protocol.
...
- **P4 Split `RuntimeContext`** *(done 2026-09-20)*: telemetry (`telemetry.py`), killswitch controller (`risk/killswitch_controller.py`), the live entry path (`execution/entries.py`) and reconciliation (`execution/reconcile.py`) are separate components; `RuntimeContext` is the wiring and job orchestration with facade properties for the old names.
- **P5 Infrastructure modules** `trading_calendar` *(done)*; the `Clock` protocol was closed without code — explicit `now` parameters and the calendar module cover it.
